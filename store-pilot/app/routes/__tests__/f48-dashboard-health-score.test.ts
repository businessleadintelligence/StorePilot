...
describe("F.4.8 Dashboard Health Score Loader", () => {
  it("1. returns null health score when store is missing", async () => {
  ...
  it("2. derives health score from metrics without internal messages", async () => {
    vi.mocked(authenticate.admin).mockResolvedValue({
      session: { shop: SHOP },
    } as unknown as Awaited<ReturnType<typeof authenticate.admin>>);
    ...
    vi.mocked(getStoreSyncStatus).mockResolvedValue({
      ...
      orders: { synced: true, count: 173, lastSyncAt: null, blocked: false, blockedReason: null },
    });
    ...
    expect(data.healthScore).toEqual({
      score: 84,
      grade: "B",
      productsScore: 20,
      inventoryScore: 24,
    ...
    expect(JSON.stringify(data.healthScore)).not.toContain("GraphQL");
  });
});
