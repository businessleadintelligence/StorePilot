...
export function calculateExecutiveCooScores(input: {
  revenue30: number;
  revenue90: number;
  revenueGrowthRate: number;
  aov: number;
  ...
  const businessHealthScore = calculateGrowthHealthScore({
  ...
  const executiveConfidenceScore = calculateGrowthScore({
    revenueGrowthRate: input.revenueGrowthRate,
    aovGrowthRate: input.aovGrowthRate,
    repeatPurchaseRate: input.repeatPurchaseRate,
    returningCustomerRate: input.returningCustomerRate,
  ...
  const revenueOpportunity = Math.round(
    Math.max(0, 100 - executiveConfidenceScore) * 18 + input.upsellOpportunity * 8 + input.crossSellOpportunity * 6,
  );
