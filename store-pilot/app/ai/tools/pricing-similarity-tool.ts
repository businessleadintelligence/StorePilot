export function pricingSimilarityKey(input: { category: string; title: string }): string {
  return `${input.category}:${input.title.toLowerCase().trim().slice(0, 48)}`;
}

export function arePricingRecommendationsSimilar(
  left: { category: string; title: string },
  right: { category: string; title: string },
): boolean {
  if (left.category !== right.category) return false;
  return pricingSimilarityKey(left) === pricingSimilarityKey(right);
}

export function dedupeSimilarPricingRecommendations<
  T extends { category: string; title: string; confidence: number; priorityScore?: number },
>(recommendations: T[]): T[] {
  const kept: T[] = [];
  for (const candidate of recommendations) {
    const duplicate = kept.find((existing) => arePricingRecommendationsSimilar(existing, candidate));
    if (!duplicate) {
      kept.push(candidate);
      continue;
    }
