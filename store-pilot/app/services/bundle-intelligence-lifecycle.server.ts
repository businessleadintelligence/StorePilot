import type { BundleFacts } from "../ai/facts/bundle-facts";
...
export type BundleRecommendationLifecycleEvent = {
  stableId: string;
...
function verificationSucceeded(input: {
  ...
  if (verification.expectedMetric === "Dead inventory units") {
    return input.facts.deadInventoryPairCount === 0;
  }

  if (verification.expectedMetric === "Inventory units reduced") {
    const baseline = Number(input.payload.baselineInventoryReduction ?? 0);
    return baseline > 0
      ? input.facts.potentialInventoryReduction < baseline
      : input.facts.potentialInventoryReduction === 0;
  }

  if (verification.expectedMetric === "Bundle attach rate") {
    const baseline = Number(input.payload.baselineAttachRate ?? 0);
    return baseline > 0
      ? input.facts.potentialAttachRate > baseline
      : input.facts.potentialAttachRate >= 0.3;
  }

  return input.facts.bundleHealthScore >= 70;
}

export async function processBundleDiscoveryLifecycle(input: {
  storeId: string;
  subjectKey: string;
  facts: BundleFacts;
  persistence: AIPersistenceRepositories;
  now?: Date;
}): Promise<BundleRecommendationLifecycleEvent[]> {
  const events: BundleRecommendationLifecycleEvent[] = [];
  ...
  for (const record of records) {
    ...
    if (
      ...
      const reason = getBundleRecommendationExpirationReason({
        facts: input.facts,
        payload: record.payloadJson,
      });
...
export async function recordBundleMerchantRecommendationFeedback(input: {
  storeId: string;
  subjectKey: string;
  stableId: string;
