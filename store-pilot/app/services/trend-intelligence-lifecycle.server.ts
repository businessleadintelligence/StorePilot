import type { TrendFacts } from "../ai/facts/trend-facts";
...
} from "../ai/agents/trend-intelligence-expiration";

export type TrendRecommendationLifecycleEvent = {
  stableId: string;
...
function withTimeline(payload: Record<string, unknown>, patch: Record<string, unknown>) {
  return { ...payload, recommendationTimeline: { ...readTimeline(payload), ...patch } };
}

export async function processTrendIntelligenceLifecycle(input: {
  storeId: string;
  subjectKey: string;
  facts: TrendFacts;
  persistence: AIPersistenceRepositories;
  now?: Date;
}): Promise<TrendRecommendationLifecycleEvent[]> {
  const events: TrendRecommendationLifecycleEvent[] = [];
...
export async function recordTrendMerchantRecommendationFeedback(input: {
  storeId: string;
  subjectKey: string;
  stableId: string;
  ...
  if (!record) throw new Error("trend_recommendation_not_found");
