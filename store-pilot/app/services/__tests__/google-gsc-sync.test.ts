...
const STORE_ID = "store-gsc-sync-test";
...
describe("GSC sync engine integration", () => {
  beforeEach(() => {
    resetConnectorRegistry();
    resetConnectorPlatformBootstrap();
    resetGa4ConnectorDeps();
  ...
  it("syncs GSC alongside other connectors without blocking them", async () => {
    ...
    expect(result.runs.find((run) => run.connectorId === "gsc")?.status).toBe("success");
  });

  it("isolates GSC quota failures", async () => {
    configureGscConnectorDeps({
      loadIntegration: async () => integration,
      getAccessToken: async () => ({
        accessToken: "access-token",
    ...
    expect(result.metrics.gsc).toBeUndefined();
    expect(result.metrics.ga4).toBeDefined();
    expect(result.runs.find((run) => run.connectorId === "gsc")?.status).toBe("failed");
  });
});
