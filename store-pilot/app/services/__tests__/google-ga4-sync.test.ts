...
const STORE_ID = "store-sync-test";
...
describe("GA4 sync engine integration", () => {
  beforeEach(() => {
    resetConnectorRegistry();
    resetConnectorPlatformBootstrap();
    resetGa4ConnectorDeps();
  ...
  it("syncs GA4 through runAllConnectors without breaking other connectors", async () => {
    const result = await syncStoreConnectors(
      { storeId: STORE_ID, propertyId: "123456789" },
    ...
    expect(result.runs.find((run) => run.connectorId === "ga4")?.status).toBe("success");
  });

  it("isolates quota exceeded failures for GA4", async () => {
    configureGa4ConnectorDeps({
      loadIntegration: async () => integration,
      getAccessToken: async () => ({
        accessToken: "access-token",
